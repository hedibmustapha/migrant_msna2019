many_records <- read.csv("./input/migrant_msna_composite_indicator_v2.csv")
...
composite_data <- ci_data %>% 
  mutate(preexisting_vulnerability_score = ifelse(preexisting_vulnerability_index3 > 2 & !is.na(preexisting_vulnerability_index3) | 
         ...
         protection_score = ifelse(protection_index1 > 2 & !is.na(protection_index1) | 
                                   protection_index2 > 2 & !is.na(protection_index2), 
                                   pmax(protection_index1,protection_index2,na.rm = T), 
...
proportion(data = composite_data,variable = "fs_score")
proportion(data = composite_data,variable = "fs_score", by ="mantika_label",path = "./output/test_function.csv")
